...
shinyServer(function(input, output) {
	...
	simulatePilots <- function(pilotN, trueEffect, alpha, power, clinSig, totalSim){
		...
		for(i in 1:totalSim){
			  ...
			  # t-test, one sample clinical significance comparison, two-tail
	...
	proceedStats <- reactive({	
		...
		data.frame(
			Proceed = c("If effect size point estimate is greater than 0",
						...
						"If treatment mean greater than control mean (one-tail t-test)",
						"If treatment mean not equal to control mean (two-tail t-test)",
						"If treatment mean greater than clinically significant effect size (one-tail t-test)",
						"If treatment mean not equal to clinically significant effect size (two-tail t-test)",
						"Always"						
					),
	...
	clinicalSamplePilotSampleDifference <- reactive({
		round(clinSigTrialSampleSize() - averageSampleSize(), 2)
	})
	
	output$apparentCasesSaved <- renderText({
		paste('When calculating full trial sample sizes to detect the observed pilot study effect size, on average you sample <strong>', clinicalSamplePilotSampleDifference() ,'</strong> fewer cases per simulation than had you calculated the full trial sample size to detect the clinically significant effect size.<br><br>', sep='')
	})
	...
	powerStats <- reactive({	
		...
		data.frame(
			Calculation = c("Sample size needed to detect clinically significant effect size",
						"Across simulations, average sample size needed to detect pilot study effect size",
	...
	# create density plot of average sample sizes
	...
	output$sampleSizePlot <- renderPlot({
		...
		abline(v=averageSampleSize(),lty=2)
		     
		polygon(densSampleSizes(), col=lightblue, density = -1)
		
		par(mar=c(0, 0, 0, 0))
		plot.new()
		legend('center',c('Average Sample Size','Clinically Significant Sample Size','Minimum/Maximum Calculated Sample Sizes'),
	...
	fullTrialStats <- reactive({	
		# Compose data frame
		data.frame(
			Name = c("Average Sample Size", 
					"Average Group Size",
					"Average Treatment Group Mean Across Simulations",
					"Average Treatment Group SD Across Simulations",
					"Average Control Group Mean Across Simulations",
					"Average Control Group SD Across Simulations",
					"Average Effect Size Across Simulations",
					"Average Effect Size SD Across Simulations"),
  ...
  # how often over-estimate effect given positive effect
	...
	conclusionStats <- reactive({	
		# Compose data frame
		data.frame(
			Inference = c("Reject Null Hypothesis: Full trial treatment mean is not equal to control mean (two-tail t-test)",
						"Reject Null Hypothesis: Full trial treatment mean greater than control mean (one-tail t-test)",
						"Reject Null Hypothesis: Full trial treatment mean not equal to clinically significant effect size (two-tail t-test)",
						"Reject Null Hypothesis: Full trial treatment mean greater than clinically significant effect size (one-tail t-test)"						
					),
	...
	generateImplicationsProceedStats <- function(shouldProceed){
		rules <- c(
							...
							"Proceed if treatment mean greater than control mean (one-tail t-test)",						
							"Proceed if treatment mean not equal to control mean (two-tail t-test)",
							"Proceed if treatment mean greater than clinically significant effect size (one-tail t-test)",
							"Proceed if treatment mean not equal to clinically significant effect size (two-tail t-test)",
							"Proceed always"						
						)
	...
	tradeoffTable <- function(true_effect, clinical_effect, simulations_count, proceed_count, proceed_method){
		...
		#Treatment mean is not equal to control mean (two-tail t-test)
		...
		if(true_effect >= clinical_effect || true_effect < clinical_effect){
          ...
          <td class="headingCol lastCol" colspan="2">Over All Simulations Where Reject H<sub>0</sub>, Overestimate True Effect?</td>
				...
				<tr>
					<td class="lastCol leftCol">Treatment mean is not equal to control mean (two-tail t-test)</td>
          ...
          <td>', round(twotail_diff_achieved_power_full_trial, 2), '</td>
				...
				<tr>
					<td class="lastCol leftCol">Treatment mean is not equal to control mean (one-tail t-test)</td>
					<td>', conduct_trial_n, '/', simulations(), '</td>
					<td class="lastCol">', round(conduct_trial_percent, 2), '</td>
				...
				<tr>
					<td class="lastCol leftCol">Treatment mean is not equal to clinical significance level (two-tail t-test)</td>
					<td>', conduct_trial_n, '/', simulations(), '</td>
					<td class="lastCol">', round(conduct_trial_percent, 2), '</td>
      		...
      		<td>', twotail_clinsig_reject_null_n_lt_true, '/', twotail_clinsig_reject_null_n, '</td>
				...
				<tr>
					<td class="lastCol leftCol">Treatment mean is not equal to clinical significance level (one-tail t-test)</td>
					<td>', conduct_trial_n, '/', simulations(), '</td>
					<td class="lastCol">', round(conduct_trial_percent, 2), '</td>
